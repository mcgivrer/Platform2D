...
public class DemoScene extends Platform2D.AbstractScene {
    ...
    public void create(Platform2D app) {
        ...
        */
        ...
        world.addConstraint(water);


        ParticleSystem ps = new ParticleSystem("ps01")
            .setMaxNbParticles(200)
            .setPosition(0, 0)
            .setSize(world.getPlayArea().getWidth(), world.getPlayArea().getHeight() * 0.70)
            ...
            .addBehavior(new Platform2D.ParticleBehavior<ParticleSystem>() {
                ...
                public GameObject create(Platform2D.Scene s, GameObject parent) {
                    ...
                    return new GameObject(parent.getName() + "-" + parent.getNextIndex())
                        ...
                        .setStaticObject(true)
                        .addAttribute("light", 1.0f);

                }

                @Override
                public void update(Platform2D.Scene s, double elapsed, GameObject go) {
                    go.update(elapsed);
                    go.getChild().forEach(n -> {
                        GameObject c = (GameObject) n;
                        Color f = c.getAttribute("starColor", c.fillColor);
                        float light = c.getAttribute("light", 1.0f);
                        light = (float) (light * Math.random());
                        c.fillColor = darkerColor(f, light, 1.0f);
